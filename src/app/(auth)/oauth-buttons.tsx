...
export interface EnabledOAuth { google: boolean; apple: boolean }

interface Props {
  enabledOAuth: EnabledOAuth
  callbackUrl?: string
  // "Continue with X" reads naturally on both sign-in and sign-up since OAuth
  // creates the account on first use, so the same copy works in both screens.
  ctaPrefix?: string
}

export function OAuthButtons({ enabledOAuth, callbackUrl, ctaPrefix = 'Continue with' }: Props) {
  if (!enabledOAuth.google && !enabledOAuth.apple) return null
  ...
  return (
    <div className="mb-4 flex flex-col gap-2">
      {enabledOAuth.apple && (
        <button
          type="button"
          onClick={() => signIn('apple', { callbackUrl: dest })}
        ...
        >
          <AppleLogo className="h-4 w-4" /> {ctaPrefix} Apple
        </button>
      )}
      {enabledOAuth.google && (
        <button
        ...
        >
          <GoogleLogo className="h-4 w-4" /> {ctaPrefix} Google
        </button>
      )}
